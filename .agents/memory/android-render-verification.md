...
Landscape rotation photos clarified that the Odin's bright side strips were the exposed full-width background outside the centered content. Keep the outer canvas theme-aware from first paint; do not restore a permanently cream startup background to address launch continuity. Preserve readable centered content rather than stretching the cards to hide the gutters. This issue is separate from native system-bar contrast and emulator duplicate painting. Subsequent physical Odin photos on 2026-09-26 confirm the bright strips are gone in portrait dark mode, with clean Home sections and colored target icons in both themes. Further landscape photos confirm the outer canvas matches both themes, but the dark-mode landscape photo shows a light native status bar with dark icons.

The user's explicit reopen/rotation test on 2026-09-26 narrowed the status-bar problem: leave Dark selected, close and reopen in landscape → light status bar with dark icons; rotate to portrait → correct dark bar with white icons; rotate back to landscape → remains correct. Light-mode photos also show appropriate dark icons in both orientations. Treat this as evidence of appearance restoration on reopening, not permanently broken landscape styling; the exact lifecycle cause and whether closing killed the process are unverified. A rotation recovery is not proof of a startup fix. Auto-mode transitions and scrolling performance remain unestablished by these still photos.

Keep the Activity theme's system-bar attributes aligned with runtime Window/controller colors, not just the visible Window.

**Why:** AndroidX splash exit can reapply system-bar colors and icon appearance from the Activity theme after earlier lifecycle Window assignments. A fixed light post-splash theme can therefore undo a correctly resolved dark preference. Awaiting Capacitor's hide promise does not guarantee the native exit animation has finished.

**How to apply:** Resolve the saved Light/Dark/Auto selection and apply a bar-only theme overlay alongside the Window updates, after splash theme installation. Preserve splash artwork and older Android navigation contrast; avoid delayed retries as a substitute for fixing the restoration source. This approach still needs the user's native reopen reproduction to confirm it fixes the Odin, not just source-contract tests.