...
Physical-device confirmation: after a Google Play update, the user confirmed improvement and photos showed readable dashboard icons and the frequency badge on the older WebView. Reinstalling before the update became available had still delivered the old rendering. Check update availability before treating unchanged device behavior as a failed fix; dashboard confirmation does not verify dose-sheet or dark-theme rendering.